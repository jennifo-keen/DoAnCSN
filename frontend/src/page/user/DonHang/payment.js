...
const Payment = () => {
  ...
  const handleSaveChanges = async () => {
    if (recipientInfo) {
      const shippingAddress = recipientInfo.shipping_address;
      const paymentMethod = "Trả sau"; // Đặt giá trị payment_method là "Trả sau"
      ...
      try {
        const response = await fetch(`http://localhost:5000/api/order/update-recipient/${user.id}/${orderId}`, {
          method: 'PUT',
          headers: {
          ...
          body: JSON.stringify({
            ...
            phone: recipientInfo.phone,
            shipping_address: shippingAddress,
            payment_method: paymentMethod,  // Gửi payment_method là "Trả sau"
          }),
        });
        ...
        if (response.ok) {
          ...
          alert(`Không thể cập nhật thông tin! ${result.message}`);
        }
      } catch (error) {
    ...
    <div className={styles.PaymentContainer}>
      ...
      <div className={styles.PaymentMethods}>
        ...
        <select
          ...
          value="Trả sau" // Đặt giá trị mặc định là "Trả sau"
          disabled // Không cho phép thay đổi phương thức thanh toán
        >
